    ...
    # Menggunakan path lokal karena masalah dengan URL GitHub
    ...
    @st.cache_data
    def load_data(source_path): # Mengubah parameter menjadi source_path
        """Memuat dataset novel dari path lokal."""
        try:
            # Membaca dari path lokal
            df = pd.read_csv(source_path, sep=',', on_bad_lines='skip', encoding='utf-8')
            ...
            st.error(f"Error saat memuat atau memproses novels.csv dari {source_path}: {e}")
            st.stop()
    ...
    # Menggunakan DATA_PATH
    novels_df = load_data(DATA_PATH) 
